...
template <typename T>
inline constexpr bool has_out_function_v = 
...
auto output_element(std::ostream& os, const T& element, const Cont&, std::true_type)
    ...
    ->decltype(container.begin(), container.end(), os)
{
    ...
    using element_type = decay_t<decltype(* constainer.beign())>;
